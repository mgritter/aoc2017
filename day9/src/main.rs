...
named!(group_member<GroupReport>, alt!(
    group | 
    garbage
));
   
fn combine_reports( v : Vec<GroupReport> ) -> GroupReport {
    // Each group is weighted based on its depth.
    // {} is 1, {{}} is 2 + 1, {{{},{}}} is 3 + 3 + 2 + 1
    // So when we parse a group, we need to add 1 to the sum so far
    // for each subgroup it contains.
    // Then add one for the enclosing group itself.
    // Garbage counts for zero.

    let total_groups : u32 = v.iter().map( |x| (*x).num_groups ).sum();
    let total_score : u32 = v.iter().map( |x| (*x).score ).sum();
    let total_garbage : u32 = v.iter().map( |x| (*x).garbage_removed ).sum();

    GroupReport {
        num_groups : total_groups + 1,
        score : total_score + total_groups + 1,
        garbage_removed : total_garbage
    }
}
    
named!(group<GroupReport>, map!(
    delimited!(
        tag!("{"),
        separated_list!( tag!(","), group_member ),
        tag!("}")
    ),
    combine_reports
));

named!(garbage<GroupReport>, map!(
    delimited!(
        tag!("<"),
        many0!( garbage_chars ),
        tag!(">")
    ),
    |count| { GroupReport{ num_groups : 0, score : 0,
                           garbage_removed:count.iter().sum() } }
));

// Cancelled characters don't count
// I think none_of only consumes a single char, but documentation not clear.
named!(garbage_chars<u32>, alt!(
    value!(0,
           pair!(tag!("!"),take!(1))) |
    value!(1,
           none_of!( "!>" ))
...
fn summarize_group( text : &str ) -> GroupReport {
    group( text.as_bytes() ).unwrap().1
}

fn main() {
    match group( INPUT.as_bytes() ) {
        IResult::Done( _, o ) => {
            println!( "Parsed: {:?}", o );
        },
        IResult::Error( _ ) => {
...
#[cfg(test)]
mod tests {
    use super::summarize_group;

    #[test]
    fn samples() {
        let s1 = summarize_group( "{}" );
        assert_eq!(1, s1.num_groups );
        assert_eq!(1, s1.score );

        let s2 = summarize_group( "{{{}}}" );
        assert_eq!(3, s2.num_groups );
        assert_eq!(6, s2.score );

        let s3 = summarize_group( "{{},{}}" );
        assert_eq!(3, s3.num_groups );
        assert_eq!(5, s3.score );

        let s4 = summarize_group( "{{{},{},{{}}}}" );
        assert_eq!(6, s4.num_groups );
        assert_eq!(16, s4.score );

        let s5 = summarize_group( "{<a>,<a>,<a>,<a>}" );
        assert_eq!(1, s5.num_groups );
        assert_eq!(1, s5.score );

        let s8 = summarize_group( "{{<a!>},{<a!>},{<a!>},{<ab>}}" );
        assert_eq!(2, s8.num_groups );
        assert_eq!(3, s8.score );

        let g3 = summarize_group( "{<<<<>}" );
        assert_eq!(3, g3.garbage_removed );

        let g5 = summarize_group( "{<!!>}" );
        assert_eq!(0, g5.garbage_removed );

        let g_nest = summarize_group( "{{<d>,{<e{>}},<a}c>}" );
        assert_eq!(6, g_nest.garbage_removed );
