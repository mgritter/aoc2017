const INPUT : &str = "p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
...
fn parse_triple( input : &str ) -> (i32, i32, i32) {
    let trimmed = input.trim_matches( |c| c == 'p' || c == 'v' ||
                                       c == 'a' || c == '<' ||
                                       c == '=' || c == '>' );
    let mut foo = trimmed.split(",").map( |s| s.parse::<i32>().unwrap() );
    let x = foo.next().unwrap();
    let y = foo.next().unwrap();
    let z = foo.next().unwrap();
    (x,y,z)
}
...
fn integer_quadratic( a : i32, b : i32, c : i32 ) -> Roots {
    // println!( "Solve {}x^2 + {}x + {} == 0", a, b, c );
    if a == 0 {
        // bx + c == 0
        // x = -c / b
        if b == 0 {
    ...
    // Solve ax^2 + bx + c
    // equal 1/(2*a) + ( -b +/- sqrt(b^2 - 4ac) )
    // So... if it's got an integer solution, this had better be a
    // positive square
    let determinant = b * b - 4 * a * c;
    if determinant < 0 {
        return Roots::None
    }
    let sqrt = (determinant as f64).sqrt() as i32;
    if sqrt * sqrt != determinant {
        return Roots::None
    }
    // We are interested in the smallest positive solution
    // so fractions are right out.
    let r1 = -b + sqrt;
    let r2 = -b - sqrt;
    if r1 * a >= 0 && r1 % (2*a) == 0 {
        if r2 * a >= 0 && r2 % ( 2*a) == 0 {
            if r1 == r2 {
                return Roots::One( r1 / (2*a) );
            } else {
                return Roots::Two( min( r1 / (2*a), r2 / (2*a) ),
                                   max( r1 / (2*a), r2 / (2*a) ) );
            }
        } else {
            return Roots::One( r1 / (2*a) );
...
// This is ghastly.
fn combine( x : Roots, y : Roots ) -> Roots {
    match (x,y) {
        (Roots::Any,x) => x,
        (x,Roots::Any) => x,        
        (Roots::None,_) => Roots::None,
        (_,Roots::None) => Roots::None,
        (Roots::One(a), Roots::One(b)) if a == b => Roots::One( a ),
        (Roots::Two(a,_), Roots::One(c)) if a == c => Roots::One( a ),
        (Roots::Two(_,b), Roots::One(c)) if b == c => Roots::One( b ),
        (Roots::One(c), Roots::Two(a,_)) if a == c => Roots::One( a ),
        (Roots::One(c), Roots::Two(_,b)) if b == c => Roots::One( b ),
        (Roots::Two(a,b), Roots::Two(c,d)) if a == c && b == d
            => Roots::Two( a, b ),
        (Roots::Two(a,b), Roots::Two(c,d)) if a == d && b == c
            => Roots::Two( a, b ),
        (Roots::Two(a,_), Roots::Two(c,_)) if a == c => Roots::One( a ),
        (Roots::Two(a,_), Roots::Two(_,d)) if a == d => Roots::One( a ),
        (Roots::Two(_,b), Roots::Two(c,_)) if b == c => Roots::One( b ),
        (Roots::Two(_,b), Roots::Two(_,d)) if b == d => Roots::One( b ),
        _ => Roots::None
    }
}

fn intercept_time( p0: (i32,i32,i32),
                   v0: (i32,i32,i32),
                   a0: (i32,i32,i32),
                   p1: (i32,i32,i32),
                   v1: (i32,i32,i32),
                   a1: (i32,i32,i32) ) -> Option<i32> {
    // pos_n = a/2 * n^2 + (v+a/2)*n + p
    // So we'll always multiply all the coefficients by two to
    // get integer arithmetic
    // pos'_n = a * n^2 + (2v+a)*n + 2*p
    let r_x : Roots =
        integer_quadratic( a0.0 - a1.0,
                           ( 2 * v0.0 + a0.0 ) - ( 2 * v1.0 + a1.0 ),
                           2 * p0.0 - 2 * p1.0 );
    let r_y : Roots =
        integer_quadratic( a0.1 - a1.1,
                           ( 2 * v0.1 + a0.1 ) - ( 2 * v1.1 + a1.1 ),
                           2 * p0.1 - 2 * p1.1 );
    let r_z : Roots =
        integer_quadratic( a0.2 - a1.2,
                           ( 2 * v0.2 + a0.2 ) - ( 2 * v1.2 + a1.2 ),
                           2 * p0.2 - 2 * p1.2 );
    // println!( "Particles: {:?} {:?} Roots: {:?} {:?} {:?}",
    //           p0, p1, r_x, r_y, r_z );

    match combine( r_x, combine( r_y, r_z ) ) {
        Roots::None => None,
        Roots::One( n ) => Some( n ),
        Roots::Two( a, b ) => Some( min(a,b) ),
        Roots::Any => Some( 0 )
    }
}
...
fn main() {
    // This is a very inefficient and complicated way to solve the
    // problem; most winning solutions just simulated until things seem
    // to have settled down.  But I though it would be fun to work things
    // out the hard way.
    
    let mut min_accel : Option<(i32,i32,i32)> = None;
    let mut min_particle : Option<usize> = None;
    let mut particle = 0;
    
    let mut position : Vec<(i32,i32,i32)> = Vec::new();
    let mut velocity : Vec<(i32,i32,i32)> = Vec::new();
    let mut accel : Vec<(i32,i32,i32)> = Vec::new();
    
    for line in INPUT.split( "\n" ) {
        let mut tok = line.split( ">, " );
        let p = parse_triple( tok.next().unwrap() );
        let v = parse_triple( tok.next().unwrap() );
        let a = parse_triple( tok.next().unwrap() );
        
        position.push( p );
        velocity.push( v );
        accel.push( a );
        
        // Calculate long-term manhattan distance
        // Each turn velocity increases by a, then gets added to position
        // pos_1 = p + (v+a)
        // pos_2 = p + (v+a) + (v+2a)
        // pos_3 = p + (v+a) + (v+2a) + (v+3a)
        // pos_n = p + n*v + n*(n+1))/2*a
        //       = a/2 * n^2 + (v+a/2)*n + p


        // as n->infinity, only a matters
        // If a is tied then initial velocity matters
        // If velocity is tied then position matters
        let m_a = a.0.abs() + a.1.abs() + a.2.abs();
        let m_v = v.0.abs() + v.1.abs() + v.2.abs();
        let m_p = p.0.abs() + p.1.abs() + p.2.abs();
        // println!( "{:?} {:?} {:?} Manhattan acceleration = {}", p, v, a, m_a );
        
        let new_min = match min_accel {
            None => true,
            Some( ( o_a, o_v, o_p ) ) => 
                ( m_a < o_a ) ||
                ( m_a == o_a && m_v < o_v ) ||
                ( m_a == o_a && m_v == o_v && m_p < o_p )
        };

        if new_min {            
            min_accel = Some( ( m_a, m_v, m_p ) );
            min_particle = Some( particle );
        }
        
        particle += 1;            
    }
    
    println!( "min particle index = {}", min_particle.unwrap() );

    let num_particles = position.len();

    let mut schedule: HashMap<i32,Vec<(usize,usize)>> = HashMap::new();
    for i in 0..num_particles-1 {
        for j in (i+1)..num_particles {
            if let Some( n ) = intercept_time( position[i],
                                               velocity[i],
                                               accel[i],
                                               position[j],
                                               velocity[j],
                                               accel[j] ) {
                println!( "{} and {} intercept at time {}", i, j, n );
                schedule.entry( n ).or_insert( Vec::new() ).push( (i,j) );
            }
        }
    }

    
    let mut times : Vec<i32> = schedule.keys().cloned().collect();
    times.sort();
    let mut death : HashMap<usize,i32> = HashMap::new();
    for t in &times {
        println!( "Processing collisions at time {}", t );
        for p in schedule.get( t ).unwrap() {
            let (i,j) = *p;
            if let Some( d ) = death.get( &i ) {
                if *d != *t  {
                    // i already dead
                    continue;
                }
            }
            if let Some( d ) = death.get( &j ) {
                if *d != *t  {
                    // j already dead
                    continue;
                }
            }
            println!( "{} and {} collide", i, j );
            death.insert( i, *t );
            death.insert( j, *t );
        }
    }
    println!( "{} particles, {} annihilated, {} survived",
                  num_particles, death.len(), num_particles - death.len() );
    
}

mod test {
    use intercept_time;
    use integer_quadratic;
    use Roots;
    
    #[test]
    fn test_linear_example() {
        let p1 = ( -6, 0, 0 );
        let v1 = (  3, 0, 0 );
        let a1 = (  0 ,0, 0 );
        let p2 = ( -4, 0, 0 );
        let v2 = (  2, 0, 0 );
        let a2 = (  0, 0, 0 );
        assert_eq!( intercept_time( p1, v1, a1, p2, v2, a2 ),
                    Some(2) );
    ...
    #[test]
    fn test_quadratic() {
        assert_eq!( integer_quadratic( 0, 1, -5 ), Roots::One( 5 ) );
        assert_eq!( integer_quadratic( 0, 1, 5 ), Roots::None );
        assert_eq!( integer_quadratic( 1, 0, -9 ), Roots::One( 3 ) );
        assert_eq!( integer_quadratic( 4, -16, 16 ), Roots::One( 2 ) );
        assert_eq!( integer_quadratic( 1, -18, 32 ), Roots::Two( 2, 16 ) );
        assert_eq!( integer_quadratic( 1, 18, 32 ), Roots::None );
    }
    
    #[test]
    fn test_quadratic_intercept() {
        let p1 = (  0, 0, 0 );
        let v1 = (  0, 0, 0 );
        let a1 = (  1, 1, 1 );
        // At position 1, 3, 6, 10, 15, ...
        let p2 = ( 15, 15, 15 );
        let v2 = (  0, 0, 0 );
        let a2 = (  0, 0, 0 );
        assert_eq!( intercept_time( p1, v1, a1, p2, v2, a2 ),
        ...
        // Let's have everything collide at (0,0,0) at time 100
        // So we need a quadratic with root 100
        // (x-100)(x+100) = x^2 -200x + 10000
        //                = a/2 * n^2 + (v+a/2)*n + p
        // translates into acceleration of 2
        // initial velocity of -201
        // initial position of 100000
        //
        // Dual-root solution
        // (x-100)(x-50) = x^2 - 150x + 5000
        //    a = 2
        //    n = -151
        //    p = 5000
        let p1 = (  10000, 20000, 5000 );
        let v1 = (  -201, -402, -151 );
        let a1 = (  2, 4, 2 );
        let p2 = (  0, 0, 0 );
        let v2 = (  0, 0, 0 );
