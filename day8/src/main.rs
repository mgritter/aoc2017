const INPUT : &str = "g dec 231 if bfx > -10
k dec -567 if wfk == 0
jq inc 880 if a < 2
sh inc -828 if nkr < -5
w inc 595 if nkr > -10
...
afu dec 990 if wfk < -1202END";

// I added a bogus token to the end because it's easier to work with nom
// that way.
...
named!(if_token, tag!(" if ") );

named!(end_token, tag!("END") );

named!(sign<&[u8],i32>, do_parse!(
    sign : opt!( tag!( "-" ) ) >>
    ( match sign {
        None => 1,
        Some(_) => -1
...
named!(signed_number<&[u8],i32>, do_parse!(
    sign_val: sign >>
    numerals: digit >>
    ( str::from_utf8( numerals ).unwrap().parse::<i32>().unwrap() *
      sign_val )
));

named!(operation<&[u8],Operation>, alt!(
    tag!( " inc ") => { |_| Operation::Inc } |
    tag!( " dec ") => { |_| Operation::Dec }
));

named!(comparison<&[u8],Comparison>, alt!(
    tag!( " == ") => { |_| Comparison::Equal } |
    tag!( " != ") => { |_| Comparison::NotEqual } |
    tag!( " > ") => { |_| Comparison::Gt } |
    tag!( " >= ") => { |_| Comparison::GtEqual } |
    tag!( " < ") => { |_| Comparison::Lt } |
    tag!( " <= ") => { |_| Comparison::LtEqual }
));
...
named!(instruction<&[u8],Instruction>, do_parse!(
    r1: alpha >>
        op: operation >>
        v1: signed_number >>
        if_token >>
        ...
        v2: signed_number >>
        ( Instruction {
            dest_register : utf_to_string( r1 ),
            op : op,
            amount : v1,
            control_register : utf_to_string( r2 ),
            control_comparison : cmp,
            control_value : v2
        } )
));

named!(instruction_list< Vec<Instruction> >, do_parse!(
    l: separated_list!( tag!( "\n" ), instruction ) >>
    end_token >>
    ( l )
));
...
impl RegisterFile {
    ...
    fn get( & self, name : &String ) -> i32 {
        self.registers.get( name ).cloned().unwrap_or( 0 )
    }

    fn modify( & mut self, name : &String, delta : i32 )  {
        // FIXME: get rid of the copy here?
        let entry = self.registers.entry( name.clone() ).or_insert( 0 );
        *entry += delta;

        if *entry > self.max_ever {
            self.max_ever = *entry;
        }
    }

    fn run_instruction( & mut self, instruction : &Instruction ) {
        let reg_val = self.get( &instruction.control_register );
        let cmp_val = instruction.control_value;
        println!( "{} {} {:?} {}", instruction.control_register,
                                   reg_val,
                                   instruction.control_comparison,
                                   cmp_val );
        
        let compare_matches = match instruction.control_comparison {
            Comparison::Gt => reg_val > cmp_val,
            Comparison::GtEqual => reg_val >= cmp_val,
            Comparison::Lt => reg_val < cmp_val,
            Comparison::LtEqual => reg_val <= cmp_val,
            Comparison::Equal => reg_val == cmp_val,
            Comparison::NotEqual => reg_val != cmp_val            
        };
        
        if compare_matches {
            println!( "                         => {} {:?} {}", instruction.dest_register,
                                                        instruction.op,
                                                        instruction.amount );
            match instruction.op {
                Operation::Inc => self.modify( &instruction.dest_register,
                                               instruction.amount ),
                Operation::Dec => self.modify( &instruction.dest_register,
                                               -instruction.amount )
            }
        }
    }

    fn find_max( & self ) -> ( &String, &i32 ) {
        self.registers.iter().max_by_key( |&(_,v)| *v ).unwrap()
    }
}

fn main() {
    let (_rest,instructions) = instruction_list( INPUT.as_bytes() )
        .unwrap();
    let mut r = RegisterFile::new();
    for i in instructions.iter() {
        r.run_instruction( &i );
    }
    let (reg, max) = r.find_max();
    println!( "Register {} has maximum value {}", reg, max );

    println!( "Maximum value used {}", r.max_ever );
}
