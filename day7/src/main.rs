...
named!(num_in_paren<u32>,
       map_res!(
           ws!(delimited!(char!('('),
                          digit,
                          char!(')'))),
           |o| str::from_utf8( o ).unwrap()
               .parse::<u32>() ));

// OK... so if we don't list the separator, or the prefix, as complete!
// then we get Incomplete because they might be there later.
// complete! turns them into Error instead so we know to fail this part
// entirely, or stop looking for more list elements.
//
// I am not alone in finding this less than charming, given the other
// comments on various bugs.
//
// FIXME: maybe parsing the whole input instead of line-by-line would make
// this more natural, but then we'd have to be careful about which whitespace
// we skip.
named!(comma_separated_strings< Vec<&[u8]> >,
       do_parse!(
           complete!(arrow) >>
           l : separated_list!( complete!(ws!(tag!( "," ))),
                                alpha ) >>
           (l) ));

fn utf_to_string( bytes: &[u8] ) -> String {
    String::from_utf8_lossy( bytes ).into_owned()
}

fn opt_to_string_vec( o: Option< Vec<&[u8]> > ) -> Vec<String> {
    match o {
        None => Vec::new(),
        Some( v ) => v.iter().map( |x| utf_to_string(x) ).collect()
    }
}

named!(parse_process<&[u8],(Process)>,
       do_parse!(
           n: alpha >>
           s: num_in_paren >>
           c: opt!(comma_separated_strings) >>
           ( Process { name : utf_to_string( n ),
                       weight : s,
                       children : opt_to_string_vec( c ),
                       parent: RefCell::new(None) } ) ) );

       
impl Process {
    fn from_text( text : &str ) -> Result<Process, &'static str> {
        let p = parse_process( text.as_bytes() );
        match p {
            IResult::Done( _i, o ) => Ok( o ),
            IResult::Error( _e ) => Err( "Parse Error" ),
            IResult::Incomplete( _i ) => Err( "Incomplete" )
        }
    }
}
...
impl ProcessTree {
    ...
    fn parse( & mut self, text : & str ) -> Result<(),()> {
        for line in text.split( "\n" ) {
            let parse = Process::from_text( line );
            match parse {
                Err( e ) => {
                    println!( "Parse error {} on {}", e, line );
                    return Err(());
                }
                Ok( p ) => {
                    // println!( "{}\n{:?} ", line, p );
    ...
    fn find_parents( &mut self ) {
        for parent in self.processes.values() {
            for child_name in parent.children.iter() {
                let child = self.processes.get( child_name )
                    .expect( "Child not found." );
                let mut child_name = child.parent.borrow_mut();
                *child_name = Some( parent.name.clone() );
            }
        }
    }

    // I thought I would have to add lifetime annotation here, but
    // I guess the elision rules work fine for this.
    fn find_root( & self ) -> Option<&Process> {
        for process in self.processes.values() {
            if *(process.parent.borrow()) == None {
                return Some(process);
            }
        }
        return None;
...
fn count_weight( v: &Vec<(u32,u32)>, n : u32 ) -> usize {
    return v.iter().filter( |i| (*i).0 == n ).count();
}

fn majority_and_outlier( v : &Vec<(u32,u32)> ) -> (u32, Option<u32>) {
    let first = v[0].0;
    let first_count = count_weight( v, first );
    if first_count == 1 {
        let goal_weight = v[1].0;
        let subtree_weight = v[0].1;
        return ( goal_weight, Some( goal_weight - subtree_weight ) );
    } else if first_count == v.len() {
        return ( first, None );
    } else {
        let outlier = v.iter()
            .find( |&&(total_weight,_subtree_weight)| total_weight != first )
            .expect( "Outlier not found!" );
        let goal_weight = first;
        let subtree_weight = outlier.1;
        return ( first, Some( goal_weight - subtree_weight ) );
    }
}

impl ProcessTree {
    fn check_balance( &self, root : &Process ) -> Result<u32, u32> {
        let children = root.children.iter()
            .map( |n| self.processes.get( n ).expect( "Missing child." ) );

        println!( "Checking {}", root.name );
        
        // Store total weight, and subtree weight
        let mut weights : Vec<(u32,u32)> = Vec::new();
        
        for c in children {
            match self.check_balance( c ) {
                Err( w ) => { return Err( w ) },
                Ok( w ) => {
                    println!( "Parent {} Child {} weight {} total_weight {}",
                               root.name, c.name, c.weight, w );
                    weights.push( (w, w - c.weight) );
                } 
            }
        }

        if weights.is_empty() {
            println!( "Balance OK for {}, no children, weight {}",
                       root.name, root.weight );
            return Ok( root.weight );
        }

        match majority_and_outlier( &weights ) {
            ( weight, None ) => {
                let total_weight = weight * (weights.len() as u32 ) + root.weight;
                println!( "Balance OK for {}, child weight {}, total weight {}",
                           root.name, weight, total_weight );
                Ok( total_weight )
            },
            ( weight, Some( outlier ) ) => {
                println!( "Outlier should be {} to balance {}",
                           outlier, weight );
                Err( outlier )
            }
        }
...
fn main() {
    ...
    tree.parse( INPUT ).expect( "Quitting." );
    tree.find_parents();
    let root = tree.find_root().expect( "No root found." );
    println!( "Root process = {}", root.name );

    match tree.check_balance( root ) {
        Ok( w ) => {
            println!( "Tree balanced, weight {}", w );
        },
        Err( w ) => {
            println!( "Tree unbalanced, outlier should weigh {}", w );
        }
